...
public class StatUpgradePacket {
    ...
    public StatUpgradePacket(String attrId, boolean isUpgrade) {
        this.attrId = attrId;
    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            // Turn the String back into a real Attribute
            Attribute attr = ForgeRegistries.ATTRIBUTES.getValue(new ResourceLocation(attrId));
            if (attr != null) {
                if (isUpgrade) {
                    StatUpgradeHandler.tryUpgrade(player, attr);
                } else {
                    StatUpgradeHandler.tryDowngrade(player, attr);
                }
            }
        });
